        ...
        for bckg_sample in background_samples:
            if len(bckg_sample['recording'])/bckg_sample['recording'].info['sfreq'] > 2:
                ...
                for epoch in bckg_sample['epochs'].get_data():
                    ...
                    epoch_path = '/home/migo/TUHP/Automatic-Epileptic-Seizure-Detection/TrainingEpochs/Data_only_records_with_seizures/' + bckg_sample['original_file'].split('/')[-1].removesuffix('.edf') + '_' + str(epoch_index) + '.pt'
        ...
        for sz_sample in seizure_samples:
            if len(sz_sample['recording'])/sz_sample['recording'].info['sfreq'] > 2:
                ...
                for epoch in sz_sample['epochs'].get_data():
                    epoch_tensor = torch.Tensor(epoch)
                    epoch_path = '/home/migo/TUHP/Automatic-Epileptic-Seizure-Detection/TrainingEpochs/Data_only_records_with_seizures/' + sz_sample['original_file'].split('/')[-1].removesuffix('.edf') + '_' + str(epoch_index) + '.pt'
                    torch.save(epoch_tensor, epoch_path)
